/*
 * Script Name: Fake Script Hazırlama
 * Version: v3.2.0
...
var scriptConfig = {
    ...
    translations: {
	    ...
	    'How to fill coordinates?': 'Koordinat Nasıl Doldurulsun',
            Manually: 'Manuel',
            ...
            '20:1 No-Attack': '20 Kat Üstü Oyunculara Saldırı yok',
            'Filter players 20 times bigger then yourself': 'Kendinizden 20 kat büyük oyuncuları filtreleyin',
            'Minimum Points Village': 'Küçük Puanlı Köyü Yazın',
            'Maximum Points Village': 'Yüksek Puanlı Köyü Yazın',
...
$.getScript('https://twscripts.dev/scripts/twSDK.js', async function () {
                ...
                if (coordinatesArray.length) {
                    ...
                    const popupContent = `
						<div class="ra-popup-content">
							...
							<div>
								<label for="rafakeCoordinates">${twSDK.tt('Coordinates')} ${
    ...
    function prepareContent(worldData) {
        ...
        return `
			<div class="ra-mb15">
				<div class="ra-grid ra-grid-3">
					<fieldset class="ra-fieldset">
						<legend>${twSDK.tt('How to fill coordinates?')}</legend>
						<select id="raCoordinatesField">
							<option value="manual">${twSDK.tt('Manually')}</option>
							<option value="automatic" selected>${twSDK.tt('Automatically')}</option>
						</select>
					</fieldset>
					<fieldset class="ra-fieldset">
						<legend>${twSDK.tt('How to send fakes?')}</legend>
						<select id="raSendFakesType">
							<option value="random" selected>${twSDK.tt('Randomly')}</option>
							<option value="sequential">${twSDK.tt('Sequential')}</option>
							<option value="selective_random">${twSDK.tt('Selective Random')}</option>
						</select>
					</fieldset>
					<fieldset class="ra-fieldset">
						<legend>${twSDK.tt('What to send?')}</legend>
						<select id="raWhatSend">
							<option value="custom" selected>${twSDK.tt('Custom')}</option>
							<option value="send_all">${twSDK.tt('Send all')}</option>
							<option value="selective_send_all">${twSDK.tt('Selective Send all')}</option>
			...
			<div class="ra-mb15" id="raUnitAmountsConfigurator">
				<fieldset class="ra-fieldset">
					<legend>${twSDK.tt('Choose units and amounts to send')}</legend>
			...
			<div id="raSelectPlayerTribe">
				<div class="ra-grid ra-mb15 ra-grid-3">
					<fieldset class="ra-fieldset">
						<legend>${twSDK.tt('Player (separate players using comma)')}</legend>
						${playersDropdown}
					</fieldset>
					<fieldset class="ra-fieldset">
						<legend>${twSDK.tt('Tribe (separate tribes using comma)')}</legend>
						${tribesDropdown}
					</fieldset>
					<fieldset class="ra-fieldset">
			...
			<div class="ra-mb15 ra-grid ra-grid-3" id="raAdvancedFilters">
				<fieldset class="ra-fieldset">
					...
					<label for="raEnable20To1Limit" class="ra-label">
						...
						${twSDK.tt('Filter players 20 times bigger then yourself')}
					</label>
				</fieldset>
				<fieldset class="ra-fieldset">
    ...
    function buildSelectiveSendAll() {
        ...
        let selectiveSendAllHtml = `
			<fieldset class="ra-fieldset">
				<legend>
					${twSDK.tt('Select units to send and what unit amounts to keep')}
				</legend>
