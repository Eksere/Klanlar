...
// Dünyalar için farmed (kazanılan) ve spent (harcanan) premium puanları hesaplama ve tabloyu güncelleme
function updatePremiumPointsTable(data) {
    const table = document.querySelector('#premiumDataTable');
    if (!table) return;
    ...
    data.forEach(world => {
        ...
        row.innerHTML = `
            ...
            <td>${world.purchased || 0}</td>
            <td>${world.spent || 0}</td>
            <td>${world.farmed || 0}</td>
            <td>${world.farmed - world.spent || 0}</td>
        `;
        tbody.appendChild(row);
    });
}

// Veriler toplandıktan sonra bu fonksiyon çağrılmalı
function collectWorldData(rawData) {
    const processedData = [];

    rawData.forEach(entry => {
        const worldName = entry.worldName || 'Unknown World';
        const purchased = entry.purchased || 0;
        const spent = entry.spent || 0;
        const farmed = entry.farmed || 0;
    ...
    updatePremiumPointsTable(processedData);
}
