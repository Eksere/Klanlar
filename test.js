...
function init(unitInfo) {
    ...
    const content = `
			...
			<div class="ra-flex">
				<div class="ra-flex-6">
					<div class="ra-mb15">
						<label for="nuke_unit">En Yavaş Saldırı Birimini Seç</label>
						<select id="nuke_unit">
							<option value="${unitInfo.config['axe'].speed}">Balta</option>
							<option value="${unitInfo.config['light'].speed}">Hafif Atlı</option>
							<option value="${unitInfo.config['heavy'].speed}">Ağır Atlı</option>
							<option value="${unitInfo.config['ram'].speed}" selected="selected">Şahmerdan/Mancınık</option>
						</select>
					</div>
				</div>
				<div class="ra-flex-6">
					<div class="ra-mb15">
						<label for="support_unit">En Yavaş Destek Birimini Seç</label>
						<select id="support_unit">
							<option value="${unitInfo.config['spear'].speed}">Mızrak</option>
							<option value="${unitInfo.config['sword'].speed}" selected="selected">Kılıç</option>
							<option value="${unitInfo.config['spy'].speed}">Casus</option>
							<option value="${knightSpeed}" data-option-unit="knight">Şövalye</option>
							<option value="${unitInfo.config['heavy'].speed}">Ağır Atlı</option>
							<option value="${unitInfo.config['catapult'].speed}">Şahmerdan</option>
						</select>
					</div>
				</div>
			</div>
			<div class="ra-mb15">
				<label for="target_coords">Hedef Kordinatları</label>
				<textarea id="target_coords"></textarea>
			</div>
			<div class="ra-flex">
				<div class="ra-flex-4">
					<div class="ra-mb15">
						<label for="nobel_coords">Misyoner Kordinatı</label>
						<textarea id="nobel_coords"></textarea>
					</div>
					<div class="ra-mb15">
						<label for="nobel_count">Misyoner Sayısı</label>
						<input id="nobel_count" type="text" value="1">
					</div>
				</div>
				<div class="ra-flex-4">
					<div class="ra-mb15">
						<label for="nuke_coords">Kami Kordinatı</label>
						<textarea id="nuke_coords"></textarea>
					</div>
					<div class="ra-mb15">
						<label for="nuke_count">Köye Kaç Kami Atılacak</label>
						<input id="nuke_count" type="text" value="1">
					</div>
				</div>
				<div class="ra-flex-4">
					<div class="ra-mb15">
						<label for="support_coords">Destek Koordinatları</label>
						<textarea id="support_coords"></textarea>
					</div>
					<div class="ra-mb15">
						<label for="support_count">Destek Koordinat</label>
...
let bbCode = `[table]`;
    ...
    const windowFooter = bbCode +`<small><strong>${scriptData.name} ${scriptData.version}</strong> - <a href="${scriptData.authorUrl}" target="_blank" rel="noreferrer noopener">${scriptData.author}</a> - <a href="${scriptData.helpLink}" target="_blank" rel="noreferrer noopener">Help</a></small>`;
    const windowStyle = `
		<style>
